...
export function SubstackCollection() {
  ...
  return (
    <section id="stackedin-posts" className="container mx-auto px-4 pb-12">
      <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white text-slate-950 shadow-xl shadow-slate-200/60">
        <div className="border-b border-slate-200 bg-gradient-to-br from-white via-sky-50/50 to-cyan-50/70 p-6 md:p-9">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-cyan-800">
                  Live from StackedIN
                </span>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                disabled={isFetching}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:cursor-wait disabled:opacity-70"
              >
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition hover:border-cyan-300 hover:bg-cyan-50"
              >
                Visit StackedIN
          ...
          <div className="relative mt-6 max-w-xl">
            ...
            <Input
              ...
              className="h-12 border-slate-300 bg-white pl-11 text-slate-900 placeholder:text-slate-400 focus-visible:ring-cyan-500"
            />
          </div>
        </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {[1, 2, 3, 4, 5, 6].map((item) => (
                <div key={item} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                  <Skeleton className="aspect-[16/9] bg-slate-200" />
                  <div className="space-y-3 p-5"><Skeleton className="h-5 w-4/5 bg-slate-200" /><Skeleton className="h-4 w-full bg-slate-200" /><Skeleton className="h-4 w-2/3 bg-slate-200" /></div>
                </div>
              ))}
            </div>
          ) : error ? (
            <div className="rounded-2xl border border-amber-300/20 bg-amber-300/10 p-6 text-center">
              <p className="font-bold text-amber-900">The live archive could not refresh.</p>
              <p className="mt-2 text-sm text-slate-600">You can retry here or open the publication directly.</p>
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {visiblePosts.map((post, index) => (
                <motion.article key={post.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: Math.min(index, 8) * 0.04 }}>
                  <a href={post.canonicalUrl} target="_blank" rel="noopener noreferrer" className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white transition duration-300 hover:-translate-y-1 hover:border-cyan-300 hover:shadow-xl hover:shadow-slate-200/70">
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <h3 className="mt-3 text-xl font-black leading-tight text-slate-950 transition group-hover:text-cyan-700">{post.title}</h3>
                      {post.subtitle ? <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">{post.subtitle}</p> : null}
                      <span className="mt-auto inline-flex items-center gap-2 pt-5 text-sm font-bold text-cyan-700">Read on StackedIN <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span>
                    </div>
                  </a>
                </motion.article>
              ))}
