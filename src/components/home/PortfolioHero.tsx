...
const career: CareerItem[] = [
  {
    ...
    logos: [{ src: hiscoxLogo, alt: "Hiscox" }],
  ...
  },
  {
    ...
    logos: [{ src: soleraLogo, alt: "Solera" }],
  ...
  },
  {
    ...
    logos: [{ src: accionLogo, alt: "Accion Labs" }, { src: kpmgLogo, alt: "KPMG" }, { src: dellLogo, alt: "Dell" }],
  ...
  },
  {
    ...
    logos: [{ src: wellsfargoLogo, alt: "Wells Fargo", className: "max-h-8" }],
  ...
  },
  {
    ...
    logos: [{ src: virtusaLogo, alt: "Virtusa" }, { src: qatarLogo, alt: "Qatar Airways" }],
  ...
  },
  {
    ...
    logos: [{ src: jdSportsLogo, alt: "JD Sports & Fashion" }],
  ...
  },
  {
    ...
    logos: [{ src: conduentLogo, alt: "Conduent", className: "rounded bg-[#005DAA] px-2 py-1" }, { src: metlifeLogo, alt: "MetLife" }, { src: boaLogo, alt: "Bank of America" }],
...
export function PortfolioHero() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-slate-200 bg-[#f7f8fa] pt-28 text-slate-950 dark:border-slate-800 dark:bg-slate-950 dark:text-white md:pt-32">
      <PortfolioAtmosphere />
      ...
      <div className="relative mx-auto grid w-full max-w-[1480px] gap-12 px-5 pb-20 md:px-8 lg:grid-cols-[minmax(340px,0.78fr)_minmax(0,1.22fr)] lg:gap-16 lg:pb-28 xl:px-12">
        <div className="lg:sticky lg:top-28 lg:h-fit">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/80 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-blue-700 shadow-sm backdrop-blur dark:border-blue-400/25 dark:bg-slate-900/70 dark:text-blue-300">
              <Sparkles className="h-3.5 w-3.5" /> Enterprise AI Solution Architect
            </div>
            <h1 className="max-w-xl text-5xl font-black tracking-[-0.055em] md:text-7xl xl:text-[5.5rem] xl:leading-[0.92]">
            ...
            <p className="mt-6 max-w-xl text-lg font-medium leading-8 text-slate-600 dark:text-slate-300 md:text-xl">
              I design secure, scalable, production-grade AI systems where business ambition meets engineering reality.
            </p>

            <div className="relative mt-8 overflow-hidden rounded-[2rem] border border-white/90 bg-white/70 p-2 shadow-[0_36px_90px_-42px_rgba(15,23,42,0.45)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/65">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              {[['12.6+', 'Years'], ['7', 'Career chapters'], ['4', 'Industries']].map(([value, label]) => (
                <div key={label} className="rounded-2xl border border-slate-200 bg-white/75 px-3 py-4 text-center backdrop-blur dark:border-slate-800 dark:bg-slate-900/70"><div className="text-xl font-black tracking-tight">{value}</div><div className="mt-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{label}</div></div>
              ))}
            </div>
        ...
        <div>
          ...
          <div className="relative pl-7 md:pl-10">
            <div className="absolute bottom-8 left-[7px] top-3 w-px bg-gradient-to-b from-blue-600 via-cyan-400 to-slate-200 dark:to-slate-800 md:left-[11px]" />
            <div className="space-y-5">
              {career.map((item, index) => (
                <motion.article key={`${item.company}-${item.period}`} initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.58, delay: Math.min(index, 3) * 0.05, ease: [0.22, 1, 0.36, 1] }} className="group relative rounded-[1.6rem] border border-slate-200 bg-white/80 p-5 shadow-[0_18px_60px_-40px_rgba(15,23,42,0.4)] backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-blue-300 hover:shadow-[0_28px_70px_-38px_rgba(37,99,235,0.28)] dark:border-slate-800 dark:bg-slate-900/75 md:p-7">
                  <span className="absolute -left-[34px] top-8 h-3.5 w-3.5 rounded-full border-[3px] border-[#f7f8fa] bg-blue-600 shadow-[0_0_0_5px_rgba(37,99,235,0.12)] dark:border-slate-950 md:-left-[46px]" />
                  <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex max-w-[250px] flex-wrap items-center gap-2 sm:justify-end">{item.logos.map((logo) => <div key={logo.alt} className="flex h-11 min-w-14 items-center justify-center rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 shadow-sm" title={logo.alt}><img src={logo.src} alt={`${logo.alt} logo`} className={`max-h-7 max-w-[100px] object-contain ${logo.className || ""}`} loading="lazy" /></div>)}</div>
                  </div>
